...
author: "Xiner Zhou, Shirley Galbiati, Jacob Paula"
date: "May 04, 2016"
output: 
  html_document: 
    highlight: tango
...
![Definition of "Entropy" and "Information gain"](infogain.PNG). 
...
**The question of feature selection is how many features should be included in our predictive models, and it is an issue of model complexity and will be chosen according to specific types of classification algorithms we introduce subsequently.**
...
trainX <-X[TrainIndex$Resample1,] 
trainY <-Y[TrainIndex$Resample1] 
 
testX <-X[-TrainIndex$Resample1,] 
...
CV = cv.glmnet(x=trainX[,1:10],y=trainY,family="binomial",type.measure = "class")
...
```{r, warning=FALSE, message=FALSE, cache=TRUE, fig.width=10, fig.height=8}
fit = glmnet(x=trainX[,1:10],y=trainY,family="binomial",alpha=1,lambda=Lmin)
...
b1 = b0[b0!=0] 
predictors<- data.frame(b1[order(abs(b1), decreasing = TRUE)])
colnames(predictors)[1]<-"Coef"
...
```{r,warning=FALSE, message=FALSE, cache=TRUE, fig.width=10, fig.height=8}
# Our final features selected by LASSO
tbl_df(predictors)
```
...
LASSOprediction <- predict(fit, testX[,1:10],type="class")
...
perf$value[3]<-scales::percent(prop.table(table(LASSOprediction, testY))[1,1]+prop.table(table(LASSOprediction, testY))[2,2])
                                                                    
perf$value[1]<-scales::percent(prop.table(table(LASSOprediction, testY))[2,2]/(prop.table(table(LASSOprediction, testY))[1,2]+prop.table(table(LASSOprediction, testY))[2,2]))
                                                                        
perf$value[2]<-scales::percent(prop.table(table(LASSOprediction, testY))[1,1]/(prop.table(table(LASSOprediction, testY))[1,1]+prop.table(table(LASSOprediction, testY))[2,1]))
...
# Visualize it
perf%>% ggplot(aes(x=Measure, y=value, fill=model, label=value))+
...
![Definition of Bayes-optimal classification rule](Bayes Rule.PNG). 


### Why Na?ve Bayes and Its Assumption

![Why Na?ve Bayes](Why Naive Bayes.PNG). 

### Naive Bayes Model

![Definition of Na?ve Bayes Model ](NB model 1.PNG). 
...
![Step 2 model](NB model 2.PNG). 
 


### Parameter Estimation
 
![NB estimation](NB Estimation.PNG).  
...
perf.model1 <-data.frame(par=seq(20,460,20), #tuning parameter
                  sensitivity=rep(NA,length(seq(20,460,20))),
                  specificity=rep(NA,length(seq(20,460,20))),
                  accuracy=rep(NA,length(seq(20,460,20))))
```
...
# Save as .Rdata
save(perf.model1, file="perf.model1.Rdata")
...
perf$value[6]<-scales::percent(perf.model1$accuracy[which.max(perf.model1$sensitivity)])
                                                                    
perf$value[4]<-scales::percent(perf.model1$sensitivity[which.max(perf.model1$sensitivity)])
                                                                        
perf$value[5]<-scales::percent(perf.model1$specificity[which.max(perf.model1$sensitivity)])
...
**What is the optimal number of split?**
```{r,warning=FALSE, message=FALSE, cache=TRUE, fig.width=10, fig.height=8}
perf.model2$par[which.max(perf.model2$sensitivity)]
```
 
**We have performance measure stored, Let's compare**
```{r,warning=FALSE, message=FALSE, cache=TRUE, fig.width=10, fig.height=8}
 
perf$value[9]<-scales::percent(perf.model1$accuracy[which.max(perf.model1$sensitivity)])
                                                                    
perf$value[7]<-scales::percent(perf.model1$sensitivity[which.max(perf.model1$sensitivity)])
                                                                        
perf$value[8]<-scales::percent(perf.model1$specificity[which.max(perf.model1$sensitivity)])
...
```{r,warning=FALSE, message=FALSE, cache=TRUE, fig.width=15, fig.height=12}
prune.fit<-prune(fit, cp=fit$cptable[which.max(perf.model2$sensitivity),"CP"])
 
plot(prune.fit, uniform=TRUE, main="Classification Tree")
...
**We are going to use the top 100 features selected by mutual inforamtion, because of computation time and capacity here.**
...
fit<-randomForest(y~., data=train_set, ntree=100, keep.forest=TRUE, importance=TRUE)
...
dat%>%ggplot(aes(x=order, y=imp, fill=factor(feature.class))) +
    geom_point( stat="identity") + 
  geom_text(aes(label=var.name,check_overlap=TRUE, size=100, angle=45, colour =factor(feature.class)))+
...
perf$value[12]<-scales::percent(prop.table(table(pred, test_set[,ncol(test_set)]))[1,1]+prop.table(table(pred, test_set[,ncol(test_set)]))[2,2])
                                                                    
perf$value[10]<-scales::percent(prop.table(table(pred, test_set[,ncol(test_set)]))[2,2]/(prop.table(table(pred, test_set[,ncol(test_set)]))[1,2]+prop.table(table(pred, test_set[,ncol(test_set)]))[2,2]))
                                                                        
perf$value[11]<-scales::percent(prop.table(table(pred, test_set[,ncol(test_set)]))[1,1]/(prop.table(table(pred, test_set[,ncol(test_set)]))[1,1]+prop.table(table(pred, test_set[,ncol(test_set)]))[2,1]))
...
![Pseudocode for AdaBoost](AdaBoost.PNG). 

### AdaBoost using Desicion Stump as Weak Learner

![Weak Learner](AdaBoost Weak Learner.PNG). 
...
perf$value[15]<-scales::percent(prop.table(table(pred, test_set[,ncol(test_set)]))[1,1]+prop.table(table(pred, test_set[,ncol(test_set)]))[2,2])
                                                                    
perf$value[13]<-scales::percent(prop.table(table(pred, test_set[,ncol(test_set)]))[2,2]/(prop.table(table(pred, test_set[,ncol(test_set)]))[1,2]+prop.table(table(pred, test_set[,ncol(test_set)]))[2,2]))
                                                                        
perf$value[14]<-scales::percent(prop.table(table(pred, test_set[,ncol(test_set)]))[1,1]/(prop.table(table(pred, test_set[,ncol(test_set)]))[1,1]+prop.table(table(pred, test_set[,ncol(test_set)]))[2,1]))
